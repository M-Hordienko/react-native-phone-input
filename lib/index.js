...
export default class PhoneInput extends Component {
  ...
  onChangePhoneNumber(number) {
    const actionAfterSetState = this.props.onChangePhoneNumber
      ? () => {
          this.props.onChangePhoneNumber(number, this.state.iso2);
        }
      : null;
  ...
  format(iso, text) {
    return this.props.autoFormat ? this.addSpaceAfterNumber(iso, text) : text;
  }

  addSpaceAfterNumber(iso, number) {
    var countryData = PhoneNumber.getCountryDataByCode(iso);

    if (countryData) {
      number = number.replace('+', '');

      var splitNumber = number
        .substring(countryData.dialCode.length, number.length)
        .replace(' ', '');
      var finalNumberOutput = `+${countryData.dialCode} ${splitNumber}`;

      return finalNumberOutput;
    }
  ...
  updateFlagAndFormatNumber(number, actionAfterSetState = null) {
    ...
    this.setState(
      {iso2, formattedNumber: this.format(iso2, phoneNumber)},
      actionAfterSetState,
    );
  }
      ...
      <View style={[styles.container, this.props.style]}>
        ...
        >
          <View style={{flexDirection: 'row', alignItems: 'center'}}>
            ...
            {arrowIcon && !disabled ? <View>{arrowIcon}</View> : null}
          </View>
        </TouchableWithoutFeedback>
        <View style={{flex: 1, marginLeft: this.props.offset || 10}}>
          <TextComponent
            ...
            onChangeText={text => {
              if (text.length <= this.getCountryCode().length) {
                this.onChangePhoneNumber(this.getCountryCode());
              } else {
